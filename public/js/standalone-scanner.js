/**
 * Standalone QR Scanner Implementation
 * No dependencies on driver check-in status
 */

// Global variables
let html5QrCode = null;
let isScanning = false;

// Initialize when page loads
document.addEventListener('DOMContentLoaded', function() {
  console.log("Standalone scanner script loaded");
  
  // Find elements
  const startButton = document.getElementById('start-scan');
  const stopButton = document.getElementById('stop-scan');
  const manualInput = document.getElementById('qr-manual-input');
  const manualSubmit = document.getElementById('qr-manual-submit');
  const errorMessage = document.getElementById('error-message');
  const successMessage = document.getElementById('success-message');
  
  // Create scanner instance
  try {
    html5QrCode = new Html5Qrcode("reader");
    console.log("QR scanner created successfully");
  } catch (error) {
    console.error("Failed to create QR scanner:", error);
  }
  
  // Set up button listeners
  if (startButton) {
    startButton.addEventListener('click', startScanner);
    console.log("Start button listener added");
  }
  
  if (stopButton) {
    stopButton.addEventListener('click', stopScanner);
    console.log("Stop button listener added");
  }
  
  // Set up manual entry
  if (manualInput && manualSubmit) {
    manualSubmit.addEventListener('click', function() {
      console.log("Manual submit clicked");
      const employeeId = manualInput.value.trim();
      if (employeeId) {
        console.log("Processing ID:", employeeId);
        manualInput.value = '';
        sendCheckInRequest(employeeId);
      }
    });
    console.log("Manual entry listener added");
  }
});

// Start scanner
function startScanner() {
  const errorMessage = document.getElementById('error-message');
  if (errorMessage) errorMessage.style.display = 'none';
  
  console.log("Starting scanner");
  
  try {
    const qrConfig = { fps: 10, qrbox: { width: 250, height: 250 } };
    
    html5QrCode.start(
      { facingMode: "environment" },
      qrConfig,
      onScanSuccess,
      onScanError
    ).then(() => {
      console.log("Scanner started successfully");
      isScanning = true;
      
      const startButton = document.getElementById('start-scan');
      const stopButton = document.getElementById('stop-scan');
      
      if (startButton) startButton.style.display = 'none';
      if (stopButton) stopButton.style.display = 'block';
    }).catch(error => {
      console.error("Error starting scanner:", error);
      const errorMessage = document.getElementById('error-message');
      if (errorMessage) {
        errorMessage.textContent = 'Failed to start scanner. Please check camera permissions.';
        errorMessage.style.display = 'block';
      }
    });
  } catch (error) {
    console.error("Error in startScanner:", error);
  }
}

// Stop scanner
function stopScanner() {
  console.log("Stopping scanner, isScanning:", isScanning);
  
  if (html5QrCode && isScanning) {
    html5QrCode.stop().then(() => {
      console.log("Scanner stopped successfully");
      isScanning = false;
      
      const startButton = document.getElementById('start-scan');
      const stopButton = document.getElementById('stop-scan');
      
      if (startButton) startButton.style.display = 'block';
      if (stopButton) stopButton.style.display = 'none';
    }).catch(error => {
      console.error("Error stopping scanner:", error);
    });
  }
}

// Handle successful scan
function onScanSuccess(decodedText) {
  console.log("Successful scan:", decodedText);
  
  // Play success sound
  const beep = new Audio('data:audio/mp3;base64,');
  beep.play();
  
  // Stop scanner
  stopScanner();
  
  // Send check-in request
  sendCheckInRequest(decodedText.trim());
}

// Handle scan errors
function onScanError(error) {
  // Just log errors
  console.log("QR scan error (normal during scanning):", error);
}

// Send check-in request to API
function sendCheckInRequest(employeeId) {
  console.log("Sending check-in request for ID:", employeeId);
  const errorMessage = document.getElementById('error-message');
  const successMessage = document.getElementById('success-message');
  
  // Hide previous messages
  if (errorMessage) errorMessage.style.display = 'none';
  if (successMessage) successMessage.style.display = 'none';
  
  // Validate employee ID
  if (!employeeId || employeeId.length !== 5 || !/^\d+$/.test(employeeId)) {
    console.error("Invalid employee ID:", employeeId);
    if (errorMessage) {
      errorMessage.textContent = 'Invalid employee ID. Please enter a valid 5-digit ID.';
      errorMessage.style.display = 'block';
    }
    return;
  }
  
  // Prepare check-in data
  const checkInData = {
    riderId: employeeId,
    location: 'Bus Stop',
    note: 'Checked in via QR scanner'
  };
  
  // Send API request
  fetch('/api/trips', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(checkInData),
    credentials: 'include'
  })
  .then(response => {
    if (response.ok) {
      return response.json();
    } else {
      return response.json().then(data => {
        throw new Error(data.message || 'Failed to check in employee');
      });
    }
  })
  .then(data => {
    console.log("Check-in successful:", data);
    
    if (successMessage) {
      successMessage.textContent = `Employee ${data.riderId} has been successfully checked in!`;
      successMessage.style.display = 'block';
    }
    
    // If there's a function to refresh trips, call it
    if (typeof fetchRecentCheckIns === 'function') {
      fetchRecentCheckIns();
    }
    
    // If there's a trip details section, update it
    const tripId = document.getElementById('trip-id');
    const tripEmployee = document.getElementById('trip-employee');
    const tripLocation = document.getElementById('trip-location');
    const tripTime = document.getElementById('trip-time');
    const tripNote = document.getElementById('trip-note');
    const tripDetails = document.getElementById('trip-details');
    
    if (tripId && tripEmployee && tripLocation && tripTime && tripDetails) {
      tripId.textContent = data.id;
      tripEmployee.textContent = data.riderId;
      tripLocation.textContent = data.location;
      tripTime.textContent = formatTime(data.timestamp || data.check_in_time);
      if (tripNote) tripNote.textContent = data.note || 'No note provided';
      tripDetails.style.display = 'block';
    }
  })
  .catch(error => {
    console.error("Check-in error:", error);
    
    if (errorMessage) {
      errorMessage.textContent = error.message || 'An error occurred during check-in';
      errorMessage.style.display = 'block';
    }
  });
}

// Format time function
function formatTime(time) {
  return new Date(time).toLocaleString();
}